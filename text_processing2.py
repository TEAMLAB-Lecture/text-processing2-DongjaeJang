...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    """
    digit_string = ''
    digitToAlpha = ['zero', 'one', 'two', 'three',
                    'four', 'five', 'six', 'seven', 'eight', 'nine']
    for letter in input_string:
        if letter.isdigit():
            idx = int(letter)
            digit_string += digitToAlpha[idx] + ' '
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
    ...
    for i in range(len(underscore_str)):
        current = underscore_str[i]
        if current == '_':
            continue
        else:
            if camelcase_str == '':
                camelcase_str += current.lower()
            else:
                before = underscore_str[i-1]
                if before == '_':
                    camelcase_str += current.upper()
                else:
                    camelcase_str += current.lower()
